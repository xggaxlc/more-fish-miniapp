declare interface ILaunchOption {
  /**
   * 打开小程序的路径
   */
  path: String;
...
declare interface IPage {
  ...
  /**
   * 当前页面的路径，类型为String。 pages/xxx/xxx
   */
  route ? : string;
...
declare interface IComponent {
   ...
   * 组件接受的外部样式类，参见 外部样式类
   */
  externalClasses ? : String | Array < string > ;
  /**
   * 一些组件选项，请参见文档其他部分的说明
   */
  options ? : {
...
declare namespace wx {
  ...
  export interface ICloudInitOpts {
    /**
     * 默认环境配置，传入字符串形式的环境 ID 可以指定所有服务的默认环境，传入对象可以分别指定各个服务的默认环境，见下方详细定义
     */
    env?: string | {
      /**
       * 数据库 API 默认环境配置
  ...
  export interface ICloudCallFunctionOpts extends BaseOptions {
    /**
     * 云函数名
     */
    name: string;
  ...
  export interface ICloudCallFunctionResult {
    /**
     * 通用返回结果
     */
    errMsg: string;
    ...
    result: any;
    /**
     * 云函数执行 ID，可用于在控制台查找日志
     */
    requestID: string;
  }

  export interface ICloud {
    init: (options: ICloudInitOpts) => void;
    callFunction: (options: ICloudCallFunctionOpts) => ICloudCallFunctionResult;

  }

  export var cloud :ICloud;
  ...
  export interface saveImageToPhotosAlbumOptions extends BaseOptions {
    filePath: string;
  }
  ...
  export function hideLoading(options?: BaseOptions): void;

  export interface ISelectorQuery {
    in: (component: IComponent) => ISelectorQuery;
  ...
  export function getUpdateManager(): IUpdateManager;
  ...
  export interface IGetSettingOptions extends BaseOptions {
    success?: (res: IAuthSetting) => void;
  }

  export function getSetting(opts: IGetSettingOptions): void;

  export interface ISetTabbarBadgeOptions extends BaseOptions {
    /**
     * tabBar 的哪一项，从左边算起
     */
    index: number;
  ...
  export function setTabBarBadge(opts: ISetTabbarBadgeOptions): void;

  export interface IRemoveTabbarBadgeOptions extends BaseOptions {
    /**
     * tabBar 的哪一项，从左边算起
     */
    index: number;
  }
  export function removeTabBarBadge(options: IRemoveTabbarBadgeOptions): void;
  ...
  export interface GetSystemInfoResult {
    ...
    /**
     * (仅Android) 性能等级，-2 或 0：该设备无法运行小游戏，-1：性能未知，>=1 设备性能值，该值越高，设备性能越好 (目前设备最高不到50)
     */
  ...
  export interface ReLaunchOptions extends BaseOptions {
    /**
     * 需要跳转的 tabBar 页面的路径（需在 app.json 的 tabBar 字段定义的页面），路径后不能带参数
     */
    url: string;
  ...
  export interface CanvasContext {
    ...
    /**
     *
     * @param reserve 本次绘制是否接着上一次绘制。即 reserve 参数为 false，则在本次调用绘制之前 native 层会先清空画布再继续绘制；若 reserver 参数为 true，则保留当前画布上的内容，本次调用 drawCanvas 绘制的内容覆盖在上面，默认 false。
  ...
  export interface CanvasToTempFilePathResult {
    /**
     * 生成文件的临时路径
     */
    tempFilePath: string;
  ...
  export interface CanvasToTempFilePathOptions extends BaseOptions {
     ...
     */
    quality?: number;
    success?: (result: CanvasToTempFilePathResult) => void;
  }

  /**
   * 把当前画布的内容导出生成图片，并返回文件路径
   */
  export function canvasToTempFilePath(options: CanvasToTempFilePathOptions): void;
